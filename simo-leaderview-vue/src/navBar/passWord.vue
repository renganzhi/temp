...
export default {
  ...
  methods: {
    ...
    addPswRules () {
      this.rules.password.push({ ...this.chkExist('sysUserChkOldPsw', { userId: this.id, nameKey: 'oldPassword' }), message: '旧密码错误，请重试' })
      this.addRules()
    ...
    chkExist (url, pram, tip) {
      ...
      return {
        ...
        asyncValidator: (rule, value, callback) => {
          if (value) {
            let myurl = '/user/unValidOldPassword'
            const config = {
              headers: {
    ...
    sure () {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$api.sysUserUpdatePsw({ newPassword: this.form.newPassword }, { id: this.id }).then(res => {
            this.$notify({
