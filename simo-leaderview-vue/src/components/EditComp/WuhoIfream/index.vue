...
export default {
  ...
  watch: {
    ...
    'item.CheckedVideoId': function () {
      var _this = this
      var cameraIndexCode = this.item.CheckedVideoId // 获取输入的监控点编号值，必填
        ...
        _this.oWebControl
          ...
          .then(function (oData) {
            if (oData.responseMsg.data) {
              ...
              if (_this.item.CheckedVideoId !== '') {
                var cameraIndexCode = _this.item.CheckedVideoId // 获取输入的监控点编号值，必填
                var streamMode = 0 // 主子码流标识：0-主码流，1-子码流
                var transMode = 1 // 传输协议：0-UDP，1-TCP
                var gpuMode = 0 // 是否启用GPU硬解，0-不启用，1-启用
                var wndId = -1 // 播放窗口序号（在2x2以上布局下可指定播放窗口）
