<template>
    <div class="GreenEarth" ref="GreenEarth" :style="{'width':item.width + 'px','height':item.height + 'px'}"></div>
</template>
<script>
...
export default {
  ...
  methods: {
    // 初始化
    resize: function () {
      let container = this.$refs.GreenEarth
    ...
    init: function () {
      ...
      _this.lightmesh = new THREE.Mesh(borderLight, new THREE.ShaderMaterial({
        uniforms: {
          ...
          glowColor: {
            type: 'c',
            value: new THREE.Color('rgba(18, 255, 148, 0.71)')
          }
        },
