<template>
  <div v-if="itemsShow">
    <label
      :style="item.tag === 'Hint' ? 'font-weight: bold; color: #fff;' : ''"
      >{{ item.name }}</label
    >
    ...
    <template v-if="item.tag === 'MarkLocation'">
      <div style="position:relative;left:50px;top:20px;">
        <div style="margin-bottom:15px">
          ...
          ><input type="number" name="" ref="lng" />
        </div>
        <div>
          <span style="margin-right:10px">纬度</span
          ><input type="number" name="" ref="lat" />
        </div>
        <button
          style="margin-left:35px;margin-top:20px;outline:none"
          @click="selectMarker"
        >
          查询
        </button>
    ...
    <template v-if="item.tag === 'Hint'"> </template>
    <template v-if="item.tag === 'Color'">
      ...
      <div class="color-w200">
        <Vcolor
          :data="selectedItem[item.key]"
          :key="10"
          :type="[item.key]"
          @getdata="ChildGetColor"
        ></Vcolor>
      </div>
    </template>
    <template v-if="item.tag === 'GradualColor'">
      ...
      <div
        v-show="selectedItem[item.key] !== 'true'"
        class="color-w200"
        style="width: 100px;"
      >
        <Vcolor
          :data="selectedItem[item.ColorKey]"
          :key="6"
          :type="[item.ColorKey]"
          @getdata="ChildGetColor"
        ></Vcolor>
      ...
      <div
        ...
        :style="{
          background:
            ...
            selectedItem[item.DoubleColorKey][0] +
            ',' +
            selectedItem[item.DoubleColorKey][1] +
            ')'
        }"
      >
        <div class="color-w15">
          <Vcolor
            :data="selectedItem[item.DoubleColorKey][0]"
            :key="13"
            :index="0"
            @getdata="getBarClr"
          ></Vcolor>
        ...
        <div class="color-w15" style="float: right">
          <Vcolor
            :data="selectedItem[item.DoubleColorKey][1]"
            :key="14"
            :index="1"
            @getdata="getBarClr"
          ></Vcolor>
    ...
    <template v-if="item.tag === 'ImgFile'">
      <div class="color-w200" style="position:relative">
        ...
        <label
          class="hoverTips"
          data-toggle="tooltip"
          :title="selectedItem[item.keyName]"
          style="position: absolute;float: left;height: 28px;right: 25px;width: 70px;overflow: hidden;"
          >{{ selectedItem[item.keyName] }}</label
        >
        <i
          class="icon-n-deleteNew delete_text"
    ...
    <template v-if="item.tag === 'GradientColor'">
      <div class="color-w200" style="margin-top: 8px;">
        <div
          ...
          :style="{
            background:
              'linear-gradient(45deg, ' + twoColor0 + ',' + twoColor1 + ')'
          }"
        >
          <div class="color-w15">
            <Vcolor
    ...
    <template v-if="item.tag === 'fontFamily'">
      <div class="form-group" style="height: 30px;">
        <div
          v-for="(fontitem, index) in settingData.textFontFaces"
          :key="index"
      ...
      >
        ...
        <div
          ...
          :style="{
            background:
              'linear-gradient(45deg, ' +
              selectedItem[item.key][index][0] +
              ',' +
              selectedItem[item.key][index][1] +
              ')'
          }"
        >
          <div class="color-w15">
            <Vcolor
              :data="selectedItem[item.key][index][0]"
              :index="index"
              @getdata="mygetColorStart"
            ></Vcolor>
          </div>
          <div class="color-w15" style="float: right">
            <Vcolor
              :data="selectedItem[item.key][index][1]"
              :index="index"
              @getdata="mygetGradColor"
            ></Vcolor>
          </div>
        </div>
        <div v-else>
          <div class="color-w200" style="float: left; width: 140px;">
            <Vcolor
              :data="selectedItem[item.key][index]"
              :type="item.key"
              :index="index"
              @getdata="mygetSingleColor"
            ></Vcolor>
          </div>
        </div>
    ...
    <template v-if="item.tag === 'monochromeArray'">
      ...
      >
        ...
        <i class="icon-n-add" @click="addColor3(index + 1)"></i>
        <i class="icon-n-toUp" @click="moveUp(index)"></i>
        <i class="icon-n-putin" @click="moveDown(index)"></i>
        <i class="icon-n-deleteNew" @click="delColor(index)"></i>
      </div>
    ...
    <template v-if="item.tag === 'GradientArray'">
      ...
      >
        ...
        <i class="icon-n-add" @click="addColor2(index + 1)"></i>
        <i class="icon-n-toUp" @click="moveUp(index)"></i>
        <i class="icon-n-putin" @click="moveDown(index)"></i>
        <i class="icon-n-deleteNew" @click="delColor(index)"></i>
      </div>
      ...
      <div class="form-group">
        <div
          ...
          :class="{
            ...
            'font-case': true,
            'card-case': true,
            act: selectedItem.imgSrc === item.imgSrc
          }"
        >
  ...
  methods: {
    selectMarker () {
      ...
      this.selectedItem.pointArray.forEach(element => {
        if (
          Number(this.$refs.lng.value) === element.lng &&
          Number(this.$refs.lat.value) === element.lat
        ) {
          this.markExit = true
        } else {
        }
    ...
    uploadVideo (e) {
      ...
      */
      ...
      this.uploadFile('video', formdata, function (data) {
        if (
          _this.selectedItem.chartType === 'video' ||
          _this.selectedItem.chartType === 'BulletFrame'
        ) {
    ...
    getBarClr (data) {
      ...
      this.selectedItem[this.item.DoubleColorKey].splice(
        data.index,
        1,
        data.color
      )
      if (!this.selectChange && this.chooseSameFlag) {
        this.chooseIndexs.forEach(i => {
          this.chartNum[i][this.item.DoubleColorKey].splice(
            data.index,
            1,
            data.color
          )
    ...
    ChildGetColor (data) {
      ...
      if (this.item.chartType === 'NewProgress') {
        ...
        if (data.type !== undefined) {
          if (data.ColorNum) {
            if (data.ColorNum === 1) {
              this.selectedItem[data.type] = [
                data.color,
                this.selectedItem[data.type][1]
              ]
            } else {
              this.selectedItem[data.type] = [
                this.selectedItem[data.type][0],
                data.color
              ]
            }
          } else {
    ...
    addColor (index) {
      ...
      if (this.selectedItem.ifGradual === 'false') {
        ...
        if (!this.selectChange && this.chooseSameFlag) {
          this.chooseIndexs.forEach(i => {
            this.chartNum[i][this.item.key].splice(index, 0, [
              '#c23531',
              '#c23531'
            ])
          })
        } else {
          this.selectedItem[this.item.key].splice(index, 0, [
            '#c23531',
            '#c23531'
          ])
        }
