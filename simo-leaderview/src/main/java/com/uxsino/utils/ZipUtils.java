...
public class ZipUtils {
    ...
    public static void readZipFile(String file){
        ...
        while (zipEnum.hasMoreElements()) {
            ...
            if (ze.isDirectory()) {
                ...
                if (ze.getName().contains("img")){
                    byte[] b = new byte[(int)size];
                    InputStream imgIn = zf.getInputStream(ze);
                    int readCount = 0; // 已经成功读取的字节的个数
                    while (readCount < size) {
                        readCount += imgIn.read(b, readCount, (int)size - readCount);
                    }
                    System.out.println();
    ...
    public static void deleteFiles(String filePath) {
        ...
        if (tempList != null){
            Arrays.stream(tempList).forEach(t -> {
                File temp = null;
                if (filePath.endsWith(File.separator)) {
                    temp = new File(filePath + t);
                }
                else {
                    temp = new File(filePath + File.separator + t);
                }
                if (temp.isFile()) {
                    temp.delete();
                }
                if (temp.isDirectory()) {
                    deleteFiles(filePath + "/" + t);
                }
            });
        }
//        File temp = null;
