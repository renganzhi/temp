...
public class MonitorDataService {
    ...
    public JsonModel statisticsResourceData(Long domainId, String baseNeClass, HttpSession session,String topoId) throws Exception {
        ...
        list.stream().filter(v -> Long.parseLong(v.get("value").toString()) > 0).forEach(v -> map.put(StringUtils.isEmpty(baseNeClass) ?
                BaseNeClass.valueOf(v.get("name").toString()).getText() : NeClass.valueOf(v.get("name").toString()).getText(), v.get("value")));
...
//                    value = String.format("%.2f", Double.parseDouble(value));
    ...
    public JsonModel handleSpecialIndicator(String neIds, BaseNeClass baseNeClass, String indicators, String componentName, String field) throws Exception {
        if ("runStatus".equals(indicators)) {
            ...
            JSONObject res = new JSONObject();
            JsonModel model = rpcProcessService.findVolume(neIds);
            if(!model.isSuccess()){
                return model;
            }
     ...
     * @return
     */
    public JsonModel getIndicatorValueStrTable(String neIds, BaseNeClass baseNeClass, String indicators, String[] componentName, String[] field,boolean isAddComOrIndName) throws Exception {
        String empMsg = "数据获取发生错误";
        // 返回的结果
        JSONObject result = new JSONObject();
        IndicatorTable ind = rpcProcessService.getIndicatorInfoByName(indicators);
...
//    }
    ...
    public JsonModel getMultipleCompObject(String neIds, String indicators, String[] component, String[] field, HttpSession session) throws Exception {
        ...
        //如果部件为""或者指标为cpu_usage_core，则没有部件
        if(!ObjectUtils.isEmpty(component) && "".equals(component[0]) || "cpu_usage_core".equals(indicators)) {
            componentList.add("");
        }else {
...
//        }
    ...
    public JsonModel VmwareStatistics(String neIds, String indicatorId, Integer type) throws Exception {
        ...
        if(type == 2){
            ...
            indValueQuery.setFieldShouldFilters(fieldFilters);
            indValueQuery.setFieldSize(1000);
            String type2 = "count";
            Boolean IsHistory = false;
